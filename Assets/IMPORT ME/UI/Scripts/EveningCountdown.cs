...
public class EveningCountdown : MonoBehaviour
{
    ...
    void Update()
    {
		...
		float parameter = Mathf.InverseLerp(timestamps[timestamps.Length-1],timestamps[0], GameManager.timeOfDay);
		...
		if(GameManager.timeOfDay > timestamps[timestampsLeft-1] && timestampsLeft >= 0)	{
