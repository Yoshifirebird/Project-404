...
public class UIButtonBase : CommonBase, IPointerEnterHandler, IPointerDownHandler, IPointerUpHandler {
	...
	public void OnPointerEnter(PointerEventData eventData)	{  //When cursor enters...
		...
		if((master && master.canInput) || !master)	{

		if(master)	{
		master.highlighted.exitEvent();
		master.highlighted.isClicked = false;
		}

		if(canHighlight){
		enterEvent ();
			if(master && master.canInput )
		 master.onAdjust.Invoke ();
		}
	...
	public void OnPointerDown(PointerEventData eventData)	{
		...
		if((master && master.canInput) || !master)clickEvent ();
	}
	...
	public void exitEvent()	{
		...
		if(canHighlight) OnExit.Invoke ();	//Call some events for exiting
		isClicked = false;
		highlighted = false;
